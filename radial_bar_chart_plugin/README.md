# GoodData.UI Dashboard Plugin - Radial Bar Chart
...
2.  Develop your plugin code in `src/dp_radial_bar_chart_plugin`

    The `src/dp_radial_bar_chart_plugin/Plugin.jsx` is the main plugin file where you have to register all
    your custom content. However, you can create as many new files as you want under the `src/dp_radial_bar_chart_plugin`
...
5.  Add plugin to one or more workspaces: `npm run add-plugin -- <url>`
    ...
    Run the `npm run add-plugin -- "https://your.hosting/pluginDirOfYourChoice/dp_radial_bar_chart_plugin.js"` to
...
6.  Use plugin on a dashboard: `npm run link-plugin -- <plugin-object-id>`
    ...
    If your plugin supports parameterization (see [src/dp_radial_bar_chart_plugin](./src/dp_radial_bar_chart_plugin/Plugin.tsx)) and
...
This project is setup so that all your custom code must be self-contained in the [src/dp_radial_bar_chart_plugin](./src/dp_radial_bar_chart_plugin) directory.

The [src/dp_radial_bar_chart_plugin\_engine](./src/dp_radial_bar_chart_plugin_engine) and [src/dp_radial_bar_chart_plugin\_entry](./src/dp_radial_bar_chart_plugin_entry) directories contain essential plugin boilerplate.
...
You can modify minEngineVersion and maxEngineVersion properties in `src/dp_radial_bar_chart_plugin\_entry/index`.
